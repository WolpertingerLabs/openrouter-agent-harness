...
export function monitorTool(ctx = DEFAULT_TOOL_CONTEXT) {
    return tool({
        name: 'monitor',
        description: 'Spawn a background shell command and capture its stdout/stderr line-by-line. Resolves when the process exits, the line buffer fills (default 1000, max 10000), max_duration_ms elapses (default 60s, max 10min), or the run is aborted. Use for tailing dev-server output, polling external state, or watching a build-watcher for a specific log line.',
        inputSchema: z.object({
            command: z.string().describe('Shell command to execute via /bin/sh -c.'),
            cwd: z
                .string()
                .optional()
                .describe("Working directory for the command. Resolved against the run's cwd if relative. Omit to inherit the run's cwd."),
            pattern: z
                .string()
                .optional()
                .describe('Optional JS regex (string) — when set, only lines matching the pattern are captured. Tested against each stdout/stderr line individually. Invalid regex resolves with { error: "invalid pattern: ..." }.'),
            max_lines: z
                .number()
                .int()
                .positive()
                .optional()
                .describe(`Maximum number of lines to capture before SIGTERMing the child. Default ${DEFAULT_MAX_LINES}; silently clamped to ${MAX_LINES_CAP} with a warn notification. Hitting this limit marks the result truncated.`),
            max_duration_ms: z
                .number()
                .int()
                .positive()
                .optional()
                .describe(`Maximum wall-clock duration in milliseconds. Default ${DEFAULT_MAX_DURATION_MS}; silently clamped to ${MAX_DURATION_MS_CAP} with a warn notification. Elapsing this timer SIGTERMs the child and marks the result truncated.`),
        }),
        execute: async ({ command, cwd: argCwd, pattern, max_lines, max_duration_ms, }) => {
            let regex;
