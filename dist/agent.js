...
import { CHARS_PER_TOKEN, COMPACTION_PROMPT, DEFAULT_KEEP_RECENT_TURNS, partitionMessages, resolveCompactionThresholdChars, resolveCompactionThresholdTokens, serializeMessagesForEstimate, } from './compaction.js';
...
import { createRouteCache, isPseudoModel, resolveRoute, resolveRouteCached, routeCacheKey, } from './router.js';
...
export class OpenRouterAgentRun {
     ...
     */
    async resolveCompactionModel(messages) {
     ...
     */
    async compact(reason = 'manual', preResolved) {
        if (this.#isIterating) {
        ...
        const { model: compactionModel, modelParams: compactionParams } = preResolved ?? (await this.resolveCompactionModel(rawMessages));
     ...
     */
    isOverCompactionThreshold(messages, windowModel) {
        ...
        if (tokenCounter !== undefined) {
            try {
                ...
                const thresholdTokens = resolveCompactionThresholdTokens(compactionThreshold, windowModel, modelContextWindows);
        ...
        const thresholdChars = resolveCompactionThresholdChars(compactionThreshold, windowModel, modelContextWindows);
        ...
        try {
            ...
            while (true) {
                ...
                if (isPseudoModel(model, this.opts.routers)) {
                    ...
                    const resolution = await resolveRouteCached(model, routingCtx, this.opts.routers, routeCache, logger);
                    if (resolution) {
 ...
 */
function buildRouterDecisionEvent(pseudoModel, turn, phase, resolution) {
    return {
