...
export class OpenRouterAgentRun {
    ...
    async #performCompaction(reason) {
        ...
        if (reason === 'auto' && failureCount >= COMPACTION_FAILURE_LIMIT) {
            this.opts.logger?.('error', 'Auto-compaction circuit breaker open — skipping (manual compact() resets it)', { consecutiveFailures: failureCount });
        ...
        const budgetChars = resolveSummarizerInputBudgetChars(this.opts.model, this.opts.modelContextWindows);
        let toSummarize = summarize;
        ...
        for (;;) {
            try {
                ...
                const retryable = isContextOverflowError(err) &&
                ...
                attempt += 1;
                this.opts.logger?.('warn', 'Compaction summarizer overflowed — dropping oldest items and retrying', { attempt, remainingItems: toSummarize.length });
                toSummarize = toSummarize.slice(Math.max(1, Math.floor(toSummarize.length / 4)));
                rendered = renderMessagesForSummary(toSummarize);
            }
        }
        ...
        if (nextChars > prevChars * COMPACTION_MIN_SHRINK_RATIO) {
            await this.#recordCompactionFailure(reason, failureCount);
            throw new Error(`Compaction did not shrink the history (${prevChars} → ${nextChars} chars) — original state preserved`);
        }
        const nextState = {
            ...state,
     ...
     */
    async #recordCompactionFailure(reason, current) {
        if (reason !== 'auto')
            return;
        try {
            const state = await this.stateAccessor.load();
     ...
     */
    async resolveContextWindowTokens() {
     ...
     * takes effect.
     */
    async shouldCompactForRealTokens(realInputTokens, messages) {
        ...
        const thresholdTokens = resolveCompactionThresholdTokens(undefined, this.opts.model, { [this.opts.model]: window }, {
            ...
            while (true) {
                ...
                try {
                    ...
                    if (responseCompleted && lastResponseEmpty && !sawAssistantActivity && !signal.aborted) {
                        ...
                        if (stateNow?.status !== 'interrupted') {
                ...
                if (this.opts.autoCompact && persistSession) {
                    try {
                        ...
                        const midRunState = (await this.stateAccessor.load());
...
export class EmptyModelResponseError extends Error {
    constructor() {
        super('Model returned an empty response (no text, reasoning, tool calls, or server-tool output)');
...
function isTransientCycleFailure(failedEvent, err) {
    if (err instanceof StreamStallError)
        return true;
    if (err instanceof EmptyModelResponseError)
        return true;
    if (failedEvent !== null) {
 ...
 */
function isEmptyAssistantOutput(extracted, output) {
