...
export function compileRule(rule) {
    ...
    if (parenIdx === -1) {
        ...
        throw new Error(`Invalid tool filter rule "${rule}": unknown tool name "${trimmed}". ` +
    ...
    if (!canonical) {
        throw new Error(`Invalid tool filter rule "${rule}": unknown tool name "${friendly}". ` +
            `Valid names: ${Object.keys(TOOL_NAME_LOOKUP).join(', ')}.`);
    }
 ...
 * later at first use.
 */
export function buildToolFilterCanUseTool(params) {
