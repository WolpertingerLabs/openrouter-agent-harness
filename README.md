...
| `compactionThreshold`       | `number`                             | no       | _(token-denominated; see below)_         | Phase 5.1 / 7.1: explicit threshold that triggers auto-compaction once `ConversationState.messages` crosses it. When set it **wins outright** and bypasses the Phase 7.1 real-token path (and the `/models` lookup) entirely — interpreted as raw **characters** by default, or as **tokens** when a `tokenCounter` is also supplied. When omitted, the trigger compares real `usage.inputTokens` against a token-denominated threshold (see `contextWindowTokens` / `outputReserveTokens` / `safetyBufferTokens`). Honoured only when `autoCompact !== false`.                                                                                                                                                                                                                                                                    |
| `contextWindowTokens`       | `number`                             | no       | _(live `/models` → static table → 128k)_ | Phase 7.1: explicit context-window size (tokens) for the active model. Wins over the live OR `/api/v1/models` `context_length` lookup and the static `MODEL_CONTEXT_WINDOWS` table when deriving the token threshold. The live lookup is lazy, TTL-cached, and failure-tolerant (network error → static table → 128k fallback), so compaction never gains a hard network dependency. Ignored when an explicit `compactionThreshold` is set.                                                                                                                                                                                                                                                                                                                                                                                        |
| `outputReserveTokens`       | `number`                             | no       | `20000`                                  | Phase 7.1: tokens reserved from the context window for the model's response (and, critically, for the summarizer call itself) when deriving the token threshold (`contextWindow − outputReserve − safetyBuffer`, floored at 25% of the window). Adopts the absolute-buffer shape of Claude Code / opencode. Ignored when an explicit `compactionThreshold` is set.                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `safetyBufferTokens`        | `number`                             | no       | `8000`                                   | Phase 7.1: extra slop buffer (tokens) on top of `outputReserveTokens` covering measurement drift. Ignored when an explicit `compactionThreshold` is set.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `keepRecentTurns`           | `number`                             | no       | _(token-budgeted; see below)_            | Phase 5.1 / 7.2: number of trailing **turns** preserved verbatim during compaction (a turn starts at a `user`-role message — true turn granularity since Phase 7.2; v1 counted messages). Older items are condensed into a single `developer`-role summary. When omitted, the keep tail is **token-budgeted** instead: whole turns newest-first within 25% of the model's context window, clamped 2k–8k tokens. Either way the kept tail starts at a clean turn boundary — never an orphaned `function_call_output` or unanchored reasoning item (both hard-400 on the Responses API).                                                                                                                                                                                                                                             |
...
| `autoPrune`                 | `boolean`                            | no       | `true`                                   | Phase 7.4: when `false`, disables the zero-LLM-call **tool-output prune tier** that runs before full compaction is considered. When the compaction threshold (or `pruneThreshold`) is crossed, older `function_call_output` contents are replaced in place with `[Old tool result content cleared]` (re-derivable tools) or `[Tool result stored at: <path>]` (offloaded under `logsRoot`, recoverable) markers — protecting the most recent 2 turns and the most recent ~40k tokens of tool output, and committing only when ≥20k tokens are reclaimed. If the pruned history falls back under the compaction threshold, the summarizer call is skipped entirely. Gated on `autoCompact`.                                                                                                                                         |
| `pruneThreshold`            | `number`                             | no       | _(unset)_                                | Phase 7.4: optional LOWER threshold (same currency as `compactionThreshold`) at which the prune tier fires on its own, before the compaction threshold is reached. Unset → the prune fires only when the compaction threshold itself is crossed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `pruneProtectedTools`       | `readonly string[]`                  | no       | `['skill']`                              | Phase 7.4: tool names whose outputs are never pruned. Add `'spawn_subagent'` to keep spawned-subagent results verbatim. Outputs whose tool name cannot be resolved from the paired `function_call` are always protected (fail-safe).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `compactionModel`           | `string`                             | no       | _(the run's `model`)_                    | Phase 7.5: model id used for the compaction summarizer call. Summaries are a mechanical condensation task — a cheaper/faster model often suffices (aider weak-model / opencode compaction-agent precedent).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `compaction`      | `{ reason, droppedMessages, preEstimatedTokens, postEstimatedTokens }` | Phase 7.5: an auto-compaction rewrote the persisted history during this run — emitted immediately before `stream_complete`. Manual `compact()` calls have no stream; subscribe to the `PostCompact` hook for those.     |
...
Phase 5.1 / 7.1: long-running sessions whose persisted message history approaches the active model's context window are condensed in-place by an auto-compaction pass. The library spawns a single isolated `callModel` (session id `<sessionId>:compact:<uuid>`, no tools) that summarizes the prefix into a `developer`-role message, then rewrites the persisted state to `[summary, ...verbatim user messages, ...keepTail]`. `previousResponseId` is cleared on the rewrite so the server cannot splice a stale response chain onto the new input array (see [spike 5.S1 §2d](./plans/spikes/5.S1-message-history.md)).

**When it fires (Phase 7.1).** The trigger is checked **between turns** — after every `callModel` cycle, not just at run end. It compares the server-reported real `usage.inputTokens` against a token-denominated threshold of `contextWindow − outputReserveTokens − safetyBufferTokens` (floored at 25% of the window). When the threshold is crossed mid-run, compaction is marked and performed at the top of the next cycle inside the same `iterate()` (Codex-style "mark-full → compact at the next turn"), so a long multi-input agentic run is protected before it overflows. The context window is resolved with the precedence **explicit `contextWindowTokens` → live OR `/api/v1/models` `context_length` lookup → static `MODEL_CONTEXT_WINDOWS` table (with per-run `modelContextWindows` overrides) → 128k fallback**; the live lookup is lazy, TTL-cached, and failure-tolerant (a network error falls back to the static table — compaction never gains a hard network dependency). The run-end check reuses the same signal — it fires only when the final cycle crossed the threshold and no subsequent cycle consumed the mark — and is **skipped entirely when `persistSession: false`** (no resume path → no point paying for a summary). On the first turn of a fresh session, with no real usage sample yet, the fallback estimate counts the `instructions` block and serialized tool schemas in addition to the message history.

**Where it cuts (Phase 7.2).** The keep tail is **turn-boundary-safe**: it always starts at a `user`-role message, so the rebuilt history never opens with an orphaned `function_call_output` or a reasoning item stranded from the item it anchors — both hard-400 on the Responses API. By default the tail is **token-budgeted**: whole turns are kept newest-first while their chars/4 estimate fits `min(8k, 25% of the context window)` clamped to 2k–8k tokens (`resolveKeepBudgetTokens`, opencode's exact shape). An oversized single turn that exceeds the budget alone falls back to keeping the most recent complete tool-call group (splitTurn analog) — never an orphaned fragment. Passing `keepRecentTurns` overrides the budget and keeps the last N turns verbatim at true turn granularity.

**Summarizer resilience (Phase 7.3).** The summarize call can no longer wedge a session:

- **Readable transcript input.** The prefix is rendered as a role-labelled transcript (`renderMessagesForSummary`) instead of raw `JSON.stringify` — each tool output truncated to 2k chars, image/document payloads replaced with `[image]`/`[document]` markers, and **encrypted reasoning content (`encrypted_content`) stripped entirely** (the summarizer cannot read it and would pay tokens for it; readable reasoning summaries are kept).
- **Input budget + drop-oldest retry.** The rendered input is capped to the summarizer's own window budget (`window − 20k` tokens, floored at 25% — `resolveSummarizerInputBudgetChars`) before the first attempt. If the call still fails with a context-overflow error (the API error text is the oracle — `isContextOverflowError`), the oldest quarter of the remaining items is dropped and the call retried, up to 3 times (Codex `remove_first_item` precedent). Non-overflow errors are never retried. Items dropped from the summarizer input are still removed from the history — they just don't inform the summary.
- **Inflation check.** If the rewritten state would not be meaningfully smaller than the original (post > 90% of pre, chars), the original state is preserved and the attempt recorded as a failure (Gemini's restore-if-inflated check).
- **Circuit breaker.** After 3 consecutive **auto**-compaction failures on a session, the auto-trigger stops firing: each skipped attempt logs an `'error'` and emits a `Notification` hook (`message: 'auto_compaction_breaker_open'`) instead of burning a summarizer call (Claude Code's thrashing breaker). The counter persists in the session state (`compactionFailureCount`), so cron re-invocations honor it. Manual `compact()` is always allowed — it bypasses the breaker, and any successful compaction resets the counter.

**Tool-output prune tier (Phase 7.4).** Before full compaction is even considered, the cheapest intervention runs first — a **zero-LLM-call microcompaction** of old tool outputs (Claude Code's clear-tool-results-first hierarchy; opencode's prune). When the compaction threshold (or the optional lower `pruneThreshold`) is crossed at run end, the history is walked newest→oldest and older `function_call_output` contents are replaced **in place** — the message/tool-call skeleton stays intact, so the model still sees the calls happened:

- `[Old tool result content cleared]` for re-derivable tools (`read_file`, `grep_files`, `glob`, `list_directory`, `run_command`) and for non-persistent sessions;
- `[Tool result stored at: <path>]` otherwise — the original bytes are offloaded under `<logsRoot>/<sessionId>/pruned/` so the agent can re-read them (recoverable compression).

Protected from pruning: everything in the most recent 2 turns, the most recent ~40k tokens of tool output (newest first), tools on the `pruneProtectedTools` skip-list (default `['skill']`), already-pruned markers, and outputs whose tool name cannot be resolved. The prune commits only when it reclaims ≥20k estimated tokens — pruning rewrites the prefix and therefore costs one prompt-cache miss, same as compaction, so a small prune is pure cache invalidation (it must not run every turn). On commit it clears `previousResponseId`, emits a `Notification` hook (`message: 'tool_outputs_pruned'`) and a `kind: 'prune'` transcript record; if the pruned history falls back under the compaction threshold, the summarizer call is skipped entirely.

**Summary quality & ergonomics (Phase 7.5).**

- **Structured summary.** `COMPACTION_PROMPT` v2 demands eight numbered sections (primary request & intent; key decisions & rationale; files/paths/identifiers with load-bearing snippets; errors & fixes incl. explicit user corrections; all user messages; pending tasks; current state; next step **quoted verbatim** — Claude Code's anti-drift device). Same contract: raw summary text only.
- **User messages survive verbatim.** The summarized prefix's `user`-role messages are re-inserted between the summary and the keep tail, newest-first under a 20k-token cap (`USER_MESSAGES_KEEP_TOKENS`, Codex shape) — a paraphrase is no substitute for the user's words. Prior summaries are excluded by the `[Compacted prior context]` marker, so repeat compactions never nest a summary-of-summary.
- **`compactionModel`.** Summarize on a cheaper model; defaults to the run's `model`.
- **Focused manual compaction.** `run.compact('manual', { instructions: 'the migration steps' })` appends a focus addendum to the structured prompt (Claude Code `/compact <focus>`). `compact()` now resolves to a `CompactionResult` (`{ reason, droppedMessages, preEstimatedTokens, postEstimatedTokens, summaryText }`, `null` on no-op).
- **Live observability.** Auto-compactions emit a `compaction` `AgentCoreEvent` in-stream immediately **before** `stream_complete` (reason, dropped count, pre/post estimated tokens), and every successful compaction fires a `PostCompact` hook — the symmetric bookend to `PreCompact`. Consumers that abandon the stream mid-run miss the event but still get the hook.

An explicit `compactionThreshold` **wins outright** — back-compat with v1, it bypasses the real-token path and the `/models` lookup entirely: the check is char-denominated by default, or token-denominated against a host-supplied `tokenCounter` when one is set.

| Option                | Type       | Default                                  | Description                                                                                                                                                      |
| --------------------- | ---------- | ---------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `compactionThreshold` | `number`   | _(token-denominated; see above)_         | Explicit threshold (v1 back-compat; chars, or tokens with a `tokenCounter`). When set, wins outright and bypasses the real-token path.                           |
| `contextWindowTokens` | `number`   | _(live `/models` → static table → 128k)_ | Explicit context-window size (tokens). Overrides the live lookup + static table.                                                                                 |
| `outputReserveTokens` | `number`   | `20000`                                  | Tokens reserved for the response / summarizer call when deriving the token threshold.                                                                            |
| `safetyBufferTokens`  | `number`   | `8000`                                   | Extra slop buffer (tokens) on top of the output reserve.                                                                                                         |
| `keepRecentTurns`     | `number`   | _(token-budgeted; see above)_            | Trailing **turns** preserved verbatim (turn granularity since 7.2). When omitted, the token-budgeted keep tail applies — 25% of the window clamped 2k–8k tokens. |
| `autoCompact`         | `boolean`  | `true`                                   | Suppress both the mid-run and run-end threshold checks when `false`. `compact()` still works.                                                                    |
| `autoPrune`           | `boolean`  | `true`                                   | Run the zero-LLM tool-output prune tier before the summarizer (gated on `autoCompact`). Set `false` to go straight to compaction.                                |
| `pruneThreshold`      | `number`   | _(unset)_                                | Optional **lower** threshold (same currency rules as `compactionThreshold`) that fires a prune-only pass before the compaction threshold is reached.             |
| `pruneProtectedTools` | `string[]` | `['skill']`                              | Tool names whose outputs are never pruned. Add `'spawn_subagent'` to also protect subagent results.                                                              |
| `compactionModel`     | `string`   | _(the run's `model`)_                    | Phase 7.5: cheaper model for the summarizer call.                                                                                                                |

```ts
import { OpenRouterAgentRun } from '@wolpertingerlabs/openrouter-agent-harness';

// Token-denominated default: fires when real input usage approaches the
// model's resolved window minus a ~28k reserve+buffer. The window is looked up
// live from /api/v1/models (cached), falling back to the static table offline.
const run = new OpenRouterAgentRun({
  apiKey,
  sessionId: 'demo',
  prompt: 'continue',
  model: 'openai/gpt-4o',
  // Optionally tighten the reserve for a chatty tool-using session:
  outputReserveTokens: 24_000,
  compactionModel: 'openai/gpt-4o-mini', // summarize on the cheap model
  keepRecentTurns: 8,
});
for await (const _ of run) {
  /* drain events — auto-compactions surface as { type: 'compaction', ... } */
}

// Force a compaction at any time between runs that share the sessionId —
// optionally focused, and returning the outcome.
const result = await run.compact('manual', { instructions: 'the database migration steps' });
```

The `PreCompact` lifecycle event fires right before each compaction call, and `PostCompact` (Phase 7.5) right after the rewrite is durable. Both are **audit-only** — return values are ignored and thrown errors are logged + swallowed. Typical use is archiving the about-to-be-discarded prefix to external storage so audit consumers retain it after the on-disk state is rewritten.
...
const run = new OpenRouterAgentRun({
  ...
  onHook: async (event, payload) => {
    ...
    // payload is { event, messages, keepRecentTurns? | keepBudgetTokens?, reason: 'auto' | 'manual' }
...
**Mid-run safety.** `compact()` is designed to be called between runs that share a `sessionId`, not mid-`for await`. The run iterator is single-shot and the SDK manages the in-memory `ConversationState` while a stream is active; calling `compact()` while `iterate()` is still yielding will race with the SDK's own `state.save()` calls and may corrupt the persisted JSON. Guarded at runtime — a `compact()` call from outside the iter while iteration is in flight throws synchronously. The Phase 7.1 mid-run auto-compaction does **not** violate this guard: it runs strictly _between_ `callModel` cycles (after the prior cycle's stream and `state.save()` have settled, before the next cycle's input is pulled) — exactly the safe window the guard protects. The run-end auto-trigger fires inside the iterator's `finally` block, so it runs whether the consumer drains the stream to completion OR `break`s on `stream_complete` (the generator's `return()` still runs finally).
