...
export const COMPACTION_PROMPT =
  'You are a context-compaction assistant. The user message is a role-labelled transcript of the prefix of an ongoing conversation between a user and an AI coding agent (including tool calls and truncated tool outputs) that has grown too long to keep in full. Produce a structured summary of this prefix with EXACTLY the following numbered sections, in order:\n' +
  '1. Primary request and intent — what the user is ultimately trying to accomplish, and any constraints they imposed.\n' +
  '2. Key decisions and rationale — choices made so far and why.\n' +
  '3. Files, paths, and identifiers — every file, directory, function, branch, URL, or id referenced, with short code snippets where they are load-bearing.\n' +
  '4. Errors and fixes — problems hit, how they were resolved, and any EXPLICIT corrections the user gave.\n' +
  '5. All user messages — a condensed list of every user message in the prefix, in order.\n' +
  '6. Pending tasks — work requested but not yet done.\n' +
  '7. Current state — where the work stands right now.\n' +
  '8. Next step — the immediate next action, quoting the most recent task-defining instructions VERBATIM from the conversation.\n' +
  'Omit verbose tool outputs that no longer matter. Return only the summary text — do not preface it with commentary, do not wrap it in markdown fences, do not add headings beyond the numbered sections.';
 ...
 */
export const COMPACTION_SUMMARY_MARKER = '[Compacted prior context]';
 ...
 * actual words.
 */
export const USER_MESSAGES_KEEP_TOKENS = 20_000;
...
export function resolveCompactionThresholdTokens(
  ...
  model: string,
  overrides?: Readonly<Record<string, number>>,
  reserveOpts?: { outputReserveTokens?: number; safetyBufferTokens?: number },
): number {
  if (configured !== undefined) return configured;
  ...
  if (Array.isArray(opts.tools)) {
    for (const tool of opts.tools) {
      try {
        ...
        if (typeof piece === 'string') chars += piece.length;
      } catch {
 ...
 */
export function resolveKeepBudgetTokens(contextWindowTokens?: number): number {
 ...
 */
function advanceToValidTailStart(messages: readonly unknown[], idx: number): number {
  let i = idx;
  while (
    i < messages.length &&
 ...
 */
function splitTurnCut(messages: readonly unknown[], from: number, budgetTokens: number): number {
  let acc = 0;
 ...
 */
export function partitionMessages<T>(
  messages: readonly T[],
  opts: number | PartitionMessagesOptions,
): { summarize: readonly T[]; keep: readonly T[] } {
  const options: PartitionMessagesOptions =
    typeof opts === 'number' ? { keepRecentTurns: opts } : opts;
  ...
  for (let i = 0; i < messages.length; i++) {
    if (isUserMessage(messages[i])) turnStarts.push(i);
  }
  ...
  if (options.keepRecentTurns !== undefined) {
    ...
    if (turnStarts.length === 0) {
      ...
      for (let k = turnStarts.length - 1; k >= 0; k--) {
        ...
        let turnTokens = 0;
        for (let i = start; i < cut; i++) turnTokens += estimateItemTokens(messages[i]);
        if (acc + turnTokens > budget) break;
        acc += turnTokens;
        cut = start;
      }
 ...
 */
export function collectRecentUserMessages<T>(
  messages: readonly T[],
  maxTokens: number = USER_MESSAGES_KEEP_TOKENS,
): T[] {
  const collected: T[] = [];
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    ...
    if (typeof item.content === 'string' && item.content.startsWith(COMPACTION_SUMMARY_MARKER)) {
      continue;
    }
    const tokens = Math.ceil(serializeMessagesForEstimate([item]).length / CHARS_PER_TOKEN);
    if (tokens > budget) continue;
    budget -= tokens;
    collected.push(messages[i] as T);
  }
